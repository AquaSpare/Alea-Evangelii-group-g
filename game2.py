...
def playGame(startParams):
  ...
  while not hasWon:
    ...
    if(not isDeadlock(board, player, captured)):
      ...
      if move == "ss":
          print("{} has surrended!".format(player.value))
          player = nextPlayer(player)
          hasWon = True
      else :
        toPos = move[1]
...
def getInput(board, player):
  ...
  """
  origin = input("Move piece from(Input ss if you want to surrender!): ")
  if origin == "ss" :
      move = (origin,origin)
      return move
...
def start_game():
    print("Start game!")
    startParams = (Mode.HUMAN_VS_HUMAN, Player.WHITE, Player.BLACK,'hard')
    playGame(startParams)
...
def manage_tournament():
    ...
    with open("schedule.txt", "r") as schedule:
        ...
        for match in schedule[1:]:
            ...
            startParams = (Mode.HUMAN_VS_HUMAN, Player.WHITE, Player.BLACK,'hard')
            winning_player = playGame(startParams)
           # board = INITIAL_BOARD
            ...
            if surrendered:
                ...
                scoreboard.add_score_by_name(players[winning_player], players[winning_player*-1 + 1], 1)
